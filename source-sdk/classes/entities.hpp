...
public:
	...
	entity_t* get_observer_target() {
		using original_fn = entity_t*(__thiscall*)(entity_t*);
		return (*(original_fn**)this)[294](this);
	}
	...
	const matrix_t& coordinate_frame() 	{
		static auto coord_frame = netvar_manager::get_net_var(utilities::fnv::hash("DT_BaseEntity"), utilities::fnv::hash("m_CollisionGroup")) - 0x30;
...
public:
	...
	bool is_knife() {
		return item_definition_index() != WEAPON_TASER && get_weapon_data()->type == WEAPONTYPE_KNIFE;
	}

	bool is_grenade() {
		return get_weapon_data()->type == WEAPONTYPE_GRENADE;
	}

	bool is_c4() {
		return get_weapon_data()->type == WEAPONTYPE_C4;
	}

	bool is_rifle() {
		switch (get_weapon_data()->type) {
	...
	bool is_pistol() {
		return get_weapon_data()->type == WEAPONTYPE_PISTOL;
	}
	...
	bool is_sniper() {
		...
		return get_weapon_data()->type == WEAPONTYPE_SNIPER_RIFLE;
	}
...
public:
	...
	NETVAR("DT_CSPlayer", "m_iObserverMode", observer_mode, int);
	...
	anim_layer* get_anim_overlays() {
		return *(anim_layer**)((DWORD)this + 0x2990);
	}

	std::array< float, 24 >& pose_parameter() {
		static int pose_parameter = netvar_manager::get_net_var(utilities::fnv::hash("DT_BaseAnimating"), utilities::fnv::hash("m_flPoseParameter"));
		return *reinterpret_cast<std::array<float, 24>*>(reinterpret_cast<uintptr_t>(this) + pose_parameter);
	}

	weapon_t* active_weapon() {
		auto active_weapon = read<uintptr_t>(netvar_manager::get_net_var(utilities::fnv::hash("DT_CSPlayer"), utilities::fnv::hash("m_hActiveWeapon"))) & 0xFFF;
		return reinterpret_cast<weapon_t*>(interfaces::entity_list->get_client_entity(active_weapon));
	}

	UINT* get_wearables() {
		return (UINT*)((uintptr_t)this + (netvar_manager::get_net_var(utilities::fnv::hash("DT_CSPlayer"), utilities::fnv::hash("m_hMyWearables"))));
	}

	UINT* get_weapons() {
		return (UINT*)((uintptr_t)this + (netvar_manager::get_net_var(utilities::fnv::hash("DT_CSPlayer"), utilities::fnv::hash("m_hMyWeapons"))));
	}

	int move_type() {
		static int type = netvar_manager::get_net_var(utilities::fnv::hash("DT_BaseEntity"), utilities::fnv::hash("m_nRenderMode")) + 1;
	...
	vec3_t get_bone_position(int bone) {
		...
		if (setup_bones(bone_matrices, MAXSTUDIOBONES, 256, 0.0f))
	...
	bool is_moving() {
		if (this->velocity().Length2D() != 0.0f)
			return true;
