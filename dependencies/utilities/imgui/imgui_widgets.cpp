...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (hovered)
        RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_FrameBgHovered), true, style.FrameRounding);
    else
    ...
    if (window->DC.ItemFlags & ImGuiItemFlags_MixedValue) {
        ...
        window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, GetColorU32(ImVec4(62.f / 255.f, 66.f / 255.f, 141.f / 255, alpha)), style.FrameRounding);
    }
    else if (*v) {
        if (hovered)
            RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImVec4(82.f / 255.f, 86.f / 255.f, 161.f / 255, alpha)), true, style.FrameRounding);
        else
            RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImVec4(62.f / 255.f, 66.f / 255.f, 141.f / 255, alpha)), true, style.FrameRounding);

        const float pad1 = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
            RenderCheckMark(check_bb.Min + ImVec2(pad1, pad1), GetColorU32(ImVec4(45.f / 255.f, 44.f / 255.f, 54.f / 255, alpha)), square_sz - pad1 * 2.0f);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    {
        ImU32 bg_col = GetColorU32((popup_open || hovered) ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
...
{
    ImGui::SameLine();
    ImGui::SetCursorPos(ImVec2(ImGui::GetWindowSize().x - 22.0f - ImGui::GetStyle().FramePadding.x * 2.0f, ImGui::GetCursorPosY() + 1));
...
bool ImGui::SubTabEx(const char* label, const char* icon, const bool selected, const ImVec2& size_arg)
{
    ...
    if (selected)
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(35, 34, 44));
